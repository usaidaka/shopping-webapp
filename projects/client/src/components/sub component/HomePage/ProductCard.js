...
export const ProductCard = (props) => {
  ...
  useEffect(() => {
    ...
    axios
      .get(`http://localhost:8000/api/products?page=${currentPage}&category=${props.category}&search=${props.searchInput}&sortBy[${props.sort.sortBy}]=${props.sort.orderBy}`)
      .then((res) => {
        setProductData(res.data);
        setTotalPage(Math.ceil(res.data.pagination.totalData/9))
      });
  }, [props.category, props.searchInput, props.sort.sortBy, props.sort.orderBy, currentPage])
        ...
        {productData.data?.map((product) => (
          <div className="grid grid-rows-13 lg:grid-rows-3 lg:grid-cols-3 h-[260px] lg:h-[150px] w-[160px] lg:w-[500px] rounded-md shadow-lg">
            <div className="row-span-8 lg:row-span-3 lg:col-span-1 flex justify-center">
              <img src={product.image_product} alt="product" className="h-[160px] lg:h-[150px]" />
            </div>
